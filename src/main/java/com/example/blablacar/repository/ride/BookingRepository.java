...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    @Query("""
            ...
            where b.passenger.id = :passengerId
              and (toStop.departsAt is null or toStop.departsAt >= :from)
            order by fromStop.departsAt asc
            """)
    List<Booking> findRecentByPassengerId(final long passengerId, final OffsetDateTime from);

    @Query("select b.ride.id from Booking b where b.id = :id and b.passenger.id = :passengerId")
    Optional<Long> findRideIdByIdAndPassengerId(final long id, final long passengerId);
